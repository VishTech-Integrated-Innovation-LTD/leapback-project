...
exports.restockInventoryItem = exports.deleteInventoryItem = exports.updateInventoryItem = exports.getInventoryById = exports.getAllInventory = exports.createInventoryItem = void 0;
...
const createInventoryItem = async (req, res, next) => {
    try {
        const { name, type, category, unitPrice, itemCode, stockQty, lowStockThreshold, availabilityStatus, } = req.body;
        ...
        if (itemCode) {
            const codeTaken = await models_1.Inventory.findOne({ where: { itemCode } });
            if (codeTaken) {
...
const getAllInventory = async (req, res, next) => {
    try {
        ...
        const items = await models_1.Inventory.findAll({
            where: whereClause,
...
const getInventoryById = async (req, res, next) => {
    try {
        ...
        const item = await models_1.Inventory.findByPk(id);
        if (!item) {
...
const updateInventoryItem = async (req, res, next) => {
    try {
        const { id } = req.params;
        const { name, category, unitPrice, itemCode, stockQty, lowStockThreshold, availabilityStatus, isActive, } = req.body;
        if (!id || typeof id !== 'string') {
            res.status(400).json({ message: 'Invalid or missing ID' });
            return;
        }
...
exports.restockInventoryItem = restockInventoryItem;
